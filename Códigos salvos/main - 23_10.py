...
# Configuração de CORS para permitir requisições do frontend React (localhost:3000)
app.add_middleware(
    CORSMiddleware,
...
def buscar_qualis(issn, qualis_df):
    ...
    return qualis_row.iloc[0]['Estrato'] if not qualis_row.empty else "NP"  # Ajustado para retornar "NP"
...
async def processar_artigos(file: UploadFile = File(...)):
    """Processa o XML com os artigos e busca os arquivos fixos de Qualis, JCR, SJR e Docentes."""
    ...
    qualis_file = 'C:\\Users\\Decivil\\Dropbox\\PPGEC\\Indicadores\\chatgpt\\FastAPI\\data\\Classificação Qualis 2017_2020.csv'
    jcr_file = 'C:\\Users\\Decivil\\Dropbox\\PPGEC\\Indicadores\\chatgpt\\FastAPI\\data\\JCR tabelado.xlsx'
    sjr_file = 'C:\\Users\\Decivil\\Dropbox\\PPGEC\\Indicadores\\chatgpt\\FastAPI\\data\\scimagojr 2023.csv'
    docentes_discentes_file = 'C:\\Users\\Decivil\\Dropbox\\PPGEC\\Indicadores\\chatgpt\\FastAPI\\data\\docentes_discentes_formatados.xlsx'
    ...
    return {
        ...
        "pontuacaoQualis": sanitize_float_values(pontuacao_qualis)
    }

@app.post("/recalcular_pontuacao_qualis/")
async def recalcular_pontuacao_qualis(artigos: List[Artigo]):
    """Recalcula a pontuação Qualis com base nos dados filtrados e alterados na Tabela 1."""
    artigos_df = pd.DataFrame([artigo.dict() for artigo in artigos])
    
    if artigos_df.empty:
        return {"message": "Nenhum artigo para recalcular."}

    pontuacao_qualis = calcular_pontuacao_qualis(artigos_df)
    return {"pontuacaoQualis": sanitize_float_values(pontuacao_qualis)}
